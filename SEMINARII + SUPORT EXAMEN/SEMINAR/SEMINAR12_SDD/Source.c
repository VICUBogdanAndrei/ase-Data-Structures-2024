...
nodLP* inserareLP(nodLP* cap, localitate loc)
{
	...
	nou->info.id = loc.id;
	nou->info.denumire = (char*)malloc((strldn(loc.denumire)+1) * sizeof(char));
...
void inserareArcInGraf(nodLP* cap, int idNodStart, int idNodStop)
{
	if (cap)
	{
		nodLP* nodStart = cautareDupaId(cap, idNodStart);
		nodLP* nodStop = cautareDupaId(cap, idNodStop);
		if (nodStart && nodStop)
		{
...
void main()
{
	...
	printf("Parcurgere in adancime de la nodul 1:\n");
