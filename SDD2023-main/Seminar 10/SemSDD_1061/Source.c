...
void transformaListaArce(nodLista** cap, int** mat, int nrNoduri)
{
	for (int i = 0; i < nrNoduri; i++)
...
void main()
{
	...
	nodLista* cap = NULL;
	transformaListaArce(&cap, mat, nrNoduri);
	afisareLista(cap);

	nodLista* arcMinim = arcGreutateMinima(cap);
	printf("\nArc greutate minima: (%d, %d, %d)",
		arcMinim->idNodStart, arcMinim->idNodFinal,
		arcMinim->greutateArc);
}