...
//functia de extragere din stiva
...
void afisareStiva(nodStiva* varf)
{
	...
	while (temp != NULL)
	{
		printf("\nData: %d/%d/%d, Prioritate: %d, Aplicatie: %s, Durata: %f", 
			temp->inf.data.zi, temp->inf.data.luna, temp->inf.data.an, temp->inf.prioritate, temp->inf.aplicatie, temp->inf.durata);
		temp = temp->next;
	}
}
...
int get(nodCoada** prim, nodCoada** ultim, notificare* val)
{
	if (*prim != NULL && *ultim != NULL)
	{
		...
		(*val).aplicatie = (char*)malloc((strlen((*prim)->inf.aplicatie) + 1) * sizeof(char));
...
void salvareCoadaVector(nodCoada* prim, nodCoada* ultim, notificare* vect, int* nr, char* denumire)
{
	notificare notif;
	while (get(&prim, &ultim, &notif) == 0)
	{
		if (strcmp(notif.aplicatie, denumire)==0)
		{
			vect[*nr] = notif;
				(*nr)++;
		}
...
void main()
{
	...
	printf("\n----------> COADA IN VECTOR <-------------\n");
	notificare* vect1 = (notificare*)malloc(nrNotif * sizeof(notificare));
	nr = 0;
	salvareCoadaVector(prim, ultim,vect, &nr, "WhatsApp");
	for (int i = 0; i < nr; i++)
		printf("\nData: %d/%d/%d, Prioritate: %d, Aplicatie: %s, Durata: %f",
			vect1[i].data.zi, vect1[i].data.luna, vect1[i].data.an, vect1[i].prioritate, vect1[i].aplicatie, vect1[i].durata);

	for (int i = 0; i < nr; i++)
		free(vect1[i].aplicatie);
	free(vect1);
